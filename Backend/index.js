  ...
  upload(req, res, function (err) {
    ...
    connection.beginTransaction(function(err) {
      ...
      connection.query('INSERT INTO document SET ?', data, function (error, results, fields) {
        ...
        connection.query('INSERT INTO table1 SET ?', data2, function (error, results, fields) {
          if (error) {
            return connection.rollback(function() {
              throw error;
            });
...
app.post("/login", multer().none(), (req, res, next) => {
  ...
  connection.query('SELECT * FROM `admin` WHERE `uname` = ? AND `pass` = ?', [data.uname, data.pass], (error, results, fields) => {
    ...
    if(results!='') {
      ...
      jwt.sign({ user: results}, 'privateKey.key', function(err, token) {
        ...
        res.cookie('accessToken', aesToken, { httpOnly: true, sameSite:true }).send('authorized_user') //secure:true
        //maxAge: 900000,
      });
...
app.get("/getdocuments", authToken, (req, res) => {
...
app.post("/getfulldetail", authToken,  multer().none(), (req, res) => {
  // console.log(req);
  let data = req.body['id']
  let resData = {}
  connection.query('SELECT * FROM document WHERE ID = ?',[data], (error, results, fields) => {
    if(error) throw error

    resData.document=results
    connection.query('SELECT * FROM table1 WHERE d_id = ?',[data], (error, results, fields) => {
      if(error) throw error
        // if(results) {resData.table1=results}else{resData.table1=null}
        resData.table1=results
      connection.query('SELECT * FROM table3 WHERE d_id = ?',[data], (error, results, fields) => {
        if(error) throw error
...
app.post("/search", authToken, multer().none(), (req, res) => {
  //console.log(req.body["searchKey"]);
  connection.query(`SELECT * FROM document WHERE data2 LIKE \'%${req.body["searchKey"]}%\'`,(error, results, fields) =>{
...
app.post("/deletetable1", authToken, multer().none(), (req, res) => {
  //console.log(req.body["id"]);
  connection.query(`DELETE FROM table1 WHERE t1_id=?`, [req.body["id"]], (error, results, fields) =>{
    if(error) console.log(error);
      res.json(results)
...
app.post("/deletetable2", authToken, multer().none(), (req, res) => {
  //console.log(req.body["id"]);
  connection.query(`DELETE FROM table3 WHERE t2_id=?`, [req.body["id"]], (error, results, fields) =>{
    if(error) console.log(error);
      res.json(results)
...
app.post("/addnewrecode", authToken, multer().none(), (req, res) => {
  ...
  connection.query(`INSERT INTO table1 SET ?`, [req.body], (error, results, fields) =>{
    if(error) console.log(error);
...
app.post("/addnewrecode2", authToken, multer().none(), (req, res) => {
  //console.log(req.body);
  connection.query(`INSERT INTO table3 SET ?`, [req.body], (error, results, fields) =>{
    if(error) console.log(error);
...
app.post("/updatedocument", authToken, multer().none(), (req, res) => {
  ...
  connection.query(`UPDATE document SET ? WHERE ID=?`, [data, req.body["ID"]], (error, results, fields) =>{
    if(error) console.log(error);
...
app.post("/updaterowtable1", authToken, multer().none(), (req, res) => {
  ...
  connection.query(`UPDATE table1 SET ? WHERE t1_id=?`, [data, req.body["id"]], (error, results, fields) =>{
    if(error) console.log(error);
...
app.post("/updaterowtable2", authToken, multer().none(), (req, res) => {
  ...
  connection.query(`UPDATE table3 SET ? WHERE t2_id=?`, [data, req.body["id"]], (error, results, fields) =>{
    if(error) console.log(error);
